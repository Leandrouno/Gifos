...
const getGifHtml = (gif ,clase)=>{
	...
	const abrirGif = (() => { open( gif.url, "_blank"); });	
	
	divContenedor.innerHTML = template;
	divContenedor.querySelector('#corazon').addEventListener("click", agregarFavoritos);  
	divContenedor.querySelector('.descarga').addEventListener( "click", abrirGif, false);  
...
function ampliar(evento){
	...
	const abrirGif = (() => { open( url_gif, "_blank"); });	
	
	descarga.addEventListener( "click", abrirGif, false);
